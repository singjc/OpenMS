...
namespace OpenMS
{
  ...
  bool read_available_memory_linux(size_t& mem_available)
  {
    ...
    const UInt64 available_kb = static_cast<UInt64>(available_pages) * static_cast<UInt64>(page_size) / 1024ull;
  ...
  bool SysInfo::getFreeSystemMemory(size_t& mem_available)
  {
    ...
    const UInt64 available_pages = static_cast<UInt64>(vm_stat.free_count) + static_cast<UInt64>(vm_stat.inactive_count);
