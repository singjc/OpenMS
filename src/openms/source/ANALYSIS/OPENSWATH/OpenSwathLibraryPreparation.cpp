...
namespace OpenMS
{
  namespace
  {
                           ...
                           const TargetedExperiment& targeted_exp)
    {
      ...
      {
        // Heavy source formats (notably TraML) may carry arbitrary or repeated native
        // transition IDs. Convert through the LightTargetedExperiment writer boundary so
        // those source IDs are canonicalized exactly once before entering persistent PQP.
        OpenSwath::LightTargetedExperiment light_exp;
