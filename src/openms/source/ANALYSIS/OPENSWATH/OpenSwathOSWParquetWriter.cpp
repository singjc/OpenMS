...
namespace OpenMS
{
  namespace
  {
    ...
    int64_t resolveTransitionId_(const OpenSwathOSWWriter::OSWValue& value,
                                 ...
                                 const std::unordered_set<int64_t>& valid_transition_ids)
    {
      ...
      if (oswValueToInt64_(value, transition_id) && valid_transition_ids.contains(transition_id))
      {
                                        ...
                                        const OpenSwathLibraryIDNormalizer::SourceIDMapping* source_ids) const
  {
    ...
    writeRows_(output_path, assay_library, osw_output, run_id, input_filename, enable_uis_scoring, source_ids);
  }
  ...
  void OpenSwathOSWParquetWriter::write(const std::string& output_path,
                                        ...
                                        bool enable_uis_scoring) const
  {
    writeRows_(output_path, assay_library, osw_output, run_id, input_filename, enable_uis_scoring, nullptr);
  }

  void OpenSwathOSWParquetWriter::writeRows_(const std::string& output_path,
                                             const OpenSwath::LightTargetedExperiment& assay_library,
                                             const OpenSwathOSWWriter::OSWData& osw_output,
                                             UInt64 run_id,
                                             const std::string& input_filename,
                                             ...
                                             const OpenSwathLibraryIDNormalizer::SourceIDMapping* source_ids) const
  {
    ...
    valid_transition_ids.reserve(transition_to_id.size());
    for (const auto& [_, transition_id] : transition_to_id)
    {
    ...
    for (const auto& transition_row : osw_output.feature_transition_rows)
    {
      ...
      const int64_t transition_id_value = resolveTransitionId_(transition_row[1], transition_to_id, valid_transition_ids);
