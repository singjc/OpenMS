...
START_SECTION([EXTRA] prepareEmpiricalLibraryToPQP preserves decoy flags when heavy TraML fallback is normalized to PQP)
{
  ...
  assay_params.enable_ipf = false;
  assay_params.unimod_file.clear();
  // Force the assay-prepared intermediate to be empty so this test
  // deterministically exercises direct decoy generation from heavy TraML.
  assay_params.min_transitions = 1000;
  assay_params.max_transitions = 1000;

  auto decoy_params = makeDeterministicDecoyParameters_();
  // Match the established OpenSwathDecoyGenerator TraML pseudo-reverse path.
  decoy_params.switch_kr = false;
  decoy_params.enable_detection_specific_losses = false;
  decoy_params.enable_detection_unspecific_losses = false;
  ...
  const auto stats = prep.prepareEmpiricalLibraryToPQP(
    toppDataPath_("OpenSwathDecoyGenerator_input.TraML"),
    FileTypes::TRAML,
    output_pqp,
    assay_params,
    decoy_params);
