...
START_SECTION(void write(const std::string&, const OpenSwath::LightTargetedExperiment&, const OpenSwathOSWWriter::OSWData&, UInt64, const std::string&, bool))
  OpenSwath::LightTargetedExperiment lib;
  OpenSwath::LightCompound c1; c1.id = "0"; c1.charge = 2; c1.rt = 10.0; lib.compounds.push_back(c1);
  OpenSwath::LightCompound c2; c2.id = "7"; c2.charge = 1; c2.rt = 20.0; lib.compounds.push_back(c2);
  OpenSwath::LightTransition t1; t1.transition_name = "3"; t1.peptide_ref = "0"; t1.product_mz = 100.0; t1.precursor_mz = 500.0; lib.transitions.push_back(t1);
  OpenSwath::LightTransition t2; t2.transition_name = "100"; t2.peptide_ref = "7"; t2.product_mz = 200.0; t2.precursor_mz = 600.0; lib.transitions.push_back(t2);
  OpenSwathLibraryIDNormalizer::validateCanonicalIDs(lib);
  ...
  const auto rows_transition_id_col = ParquetFile::getColumn(rows_transitions, "transition_id");
  std::unordered_set<int64_t> row_transition_ids;
  for (int64_t row = 0; row < rows_transitions->num_rows(); ++row)
  {
    row_transition_ids.insert(ParquetFile::getInt64(rows_transition_id_col, row, -1, false));
  }
  TEST_EQUAL(row_transition_ids.size(), 2)
  TEST_EQUAL(row_transition_ids.count(3), 1)
  TEST_EQUAL(row_transition_ids.count(100), 1)
END_SECTION

START_SECTION(void write(...) preserves sparse canonical precursor ids and explicit precursor decoy annotations)
  OpenSwath::LightTargetedExperiment lib;
  OpenSwath::LightCompound c0; c0.id = "0"; c0.charge = 2; c0.rt = 10.0; c0.setDecoy(false); lib.compounds.push_back(c0);
  OpenSwath::LightCompound c2; c2.id = "2"; c2.charge = 2; c2.rt = 20.0; c2.setDecoy(true); lib.compounds.push_back(c2);
  OpenSwath::LightCompound c7; c7.id = "7"; c7.charge = 2; c7.rt = 30.0; c7.setDecoy(false); lib.compounds.push_back(c7);
  ...
  OpenSwath::LightTransition t2; t2.transition_name = "200"; t2.peptide_ref = "2"; t2.product_mz = 200.0; t2.precursor_mz = 500.0; t2.setDetectingTransition(true); t2.setDecoy(false); lib.transitions.push_back(t2);
  OpenSwath::LightTransition t7; t7.transition_name = "300"; t7.peptide_ref = "7"; t7.product_mz = 300.0; t7.precursor_mz = 600.0; t7.setDetectingTransition(true); t7.setDecoy(false); lib.transitions.push_back(t7);
  OpenSwathLibraryIDNormalizer::validateCanonicalIDs(lib);
  ...
  Feature f7; f7.setUniqueId(3); f7.setRT(30.0); f7.setIntensity(300.0); f7.setMetaValue("PeptideRef", "7");
  Feature s7; s7.setRT(30.0); s7.setIntensity(30.0); s7.setMetaValue("FeatureLevel", "MS2"); s7.setMetaValue("native_id", "300"); f7.getSubordinates().push_back(s7);
  fmap.push_back(f0);
  fmap.push_back(f2);
  fmap.push_back(f7);
  ...
  std::map<int64_t, bool> precursor_info;
  for (int64_t row = 0; row < precursors->num_rows(); ++row)
  {
    precursor_info[ParquetFile::getInt64(precursor_id_col, row, -1, false)] =
      ParquetFile::getBool(precursor_decoy_col, row, false, true);
  }

  TEST_EQUAL(precursor_info.size(), 3)
  TEST_EQUAL(precursor_info.at(0), false)
  TEST_EQUAL(precursor_info.at(2), true)
  TEST_EQUAL(precursor_info.at(7), false)

  OpenSwathOSWParquetReader reader;
  const auto scored = reader.fetchPeakGroupFeatures(base, "ms1ms2");
  bool found_zero = false, found_two = false, found_seven = false;
  for (Size i = 0; i < scored.feature_id.size(); ++i)
  {
    ...
    {
      TEST_EQUAL(scored.precursor_id[i], 7)
      TEST_EQUAL(scored.decoy[i], false)
      found_seven = true;
    }
  }
